alter function sf_alumnosAleph (@clectivo varchar(4),@sesion varchar(1),@clase varchar(6))
returns table
as
return
(
...
(case 
when CLA.ACAD_CAREER='EXIN' OR RTRIM(LTRIM([CLA].CATALOG_NBR))='AD6'  then CONVERT(CHAR(10),DATEADD(YEAR,+1, [CLA].END_DT),112)
else CONVERT(CHAR(10),DATEADD(DAY,-2, [CLA].END_DT),112) end) collate Latin1_General_CI_AS AS F_FINAL,

(case 
when CLA.ACAD_CAREER='EXIN' OR RTRIM(LTRIM([CLA].CATALOG_NBR))='AD6' then '07'
...
WHERE 
 ...
 [CLA].STRM = [INS].STRM
		...
		*/
		...
		--AND CLA.DT
		/*
		AND CLA.END_DT = (	SELECT MAX(x.END_DT) FROM PS_CLASS_TBL x WHERE x.EMPLID=CLA.EMPLID AND x.CRSE_OFFER_NBR=CLA.CRSE_OFFER_NBR AND	 x.STRM=CLA.STRM
			AND x.SESSION_CODE=CLA.SESSION_CODE	AND x.CLASS_SECTION=CLA.CLASS_SECTION AND x.INSTITUTION=CLA.INSTITUTION	AND x.ACAD_CAREER=CLA.ACAD_CAREER
			AND x.CLASS_NBR=CLA.CLASS_NBR 	)  */

	     AND [INS].STDNT_ENRL_STATUS='E'         
        --AND [INS].STRM =@clectivo
		 --FECHA
		 AND INS.CRSE_GRADE_INPUT<>'FDA' --ULTIMA ADICIÓN POR CASO RARO DE INSCRIPCION Y FDA 07/02 y 08/02
		AND GETDATE() <= (case 
		when RTRIM(LTRIM([CLA].CATALOG_NBR))='AD6'  then DATEADD(YEAR,+1, [CLA].END_DT)
		else DATEADD(DAY,-2, [CLA].END_DT) end)
		...
		,(case 
		when CLA.ACAD_CAREER='EXIN' OR RTRIM(LTRIM([CLA].CATALOG_NBR))='AD6'  then CONVERT(CHAR(10),DATEADD(YEAR,+1, [CLA].END_DT),112)
		else CONVERT(CHAR(10),DATEADD(DAY,-2, [CLA].END_DT),112) end),
		(case 
when CLA.ACAD_CAREER='EXIN' OR RTRIM(LTRIM([CLA].CATALOG_NBR))='AD6' then '07'
else '09' end)
