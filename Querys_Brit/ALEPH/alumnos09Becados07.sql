...
WHERE 
 [CLA].STRM = [INS].STRM
       ...
       AND [FRE].EFFDT = (
        ...
        AND [INS].STRM ='2015' --CICLO LECTIVO QUE CURSA EL ALUMNO *
        AND DOC.PRIMARY_NID='Y' AND
         DIR.EFF_STATUS='A' AND ST.COUNTRY='PER'
         AND CLA.SESSION_CODE LIKE 'C%' --PARA MANEJAR LAS SESIONES LA LETRA RESPECTIVA A CADA MES ( L - DICIEMBRE) *
         AND CLA.ACAD_CAREER IN ('EXIN','CENT','CULT','ELNR','EMPR') -- CORRESPONDE A GRADO ACADEMICO *
         --AND LTRIM([CLA].CATALOG_NBR )= 'AD6' -- CORRESPONDE A LA CLASE *
