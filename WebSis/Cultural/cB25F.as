function hazBoton(titulo, link, p, btn)
{
    var _loc2 = btn.submenu.createEmptyMovieClip("tempo" + p, p);
    var color_texto = 10726317;
    var color_texto_on = 5791081;
    var my_fmt = new TextFormat();
    my_fmt.bold = false;
    my_fmt.font = "Arial";
    my_fmt.size = 10;
    my_fmt.bold = false;
    my_fmt.color = color_texto;
    var _loc3 = my_fmt.getTextExtent(titulo);
    var _loc5 = _loc3.textFieldWidth;
    var _loc7 = _loc3.textFieldHeight;
    _loc2.createTextField("titulo_txt", _loc2.getNextHighestDepth(), 0, 0, 135, 0);
    _loc2.titulo_txt.setNewTextFormat(my_fmt);
    _loc2.titulo_txt.selectable = false;
    _loc2.titulo_txt.autoSize = true;
    _loc2.titulo_txt.multiline = true;
    _loc2.titulo_txt.wordWrap = true;
    _loc2.titulo_txt.text = titulo;
    _loc2.attachMovie("linea", "linea" + p, _loc2.getNextHighestDepth());
    trace (_loc2._height);
    _loc2["linea" + p]._y = _loc2._height;
    ...
    {
        my_fmt.color = color_texto_on;
        this.titulo_txt.setTextFormat(my_fmt);
    };
...
function accion_boton(mc_animado, nom_url)
{
    mc_animado.btn.onRelease = function ()
    {
        getURL(nom_url, "");
    };
    mc_animado.onEnterFrame = function ()
    {
        ...
        {
            if (mc_animado._currentframe != num_frame_end)
            {
        ...
        else if (mc_animado._currentframe != 1)
        {
...
stop ();
var num_frame_end = 10;
accion_boton(btn_1, "Galeria_Arte.aspx");
accion_boton(btn_2, "Teatro_Britanico.aspx");
accion_boton(btn_3, "Auditorios.aspx");
accion_boton(btn_4, "Cursos_Talleres.aspx");
stop ();
var separacion = linea_1._height;
var contador = 0;
var contador2 = 0;
var contador3 = 0;
var contador4 = 0;
for (n in objMenu1.boton)
{
    if (n != "__atributos")
    {
        ++contador;
        mc = hazBoton(objMenu1.boton[n].titulo.__value, objMenu1.boton[n].link.__value, contador, btn_1);
        mc._x = 0;
        if (contador != 1)
        {
            mc._y = btn_1.submenu["tempo" + (contador - 1)]._y + btn_1.submenu["tempo" + (contador - 1)]._height;
        }
        else
        {
            mc._y = 0;
        } // end else if
        btn_1.submenu.bkg._height = mc._y + mc._height;
    } // end if
} // end of for...in
for (n in objMenu2.boton)
{
    if (n != "__atributos")
    {
        ++contador2;
        mc = hazBoton(objMenu2.boton[n].titulo.__value, objMenu2.boton[n].link.__value, contador2, btn_2);
        mc._x = 0;
        if (contador != 1)
        {
            mc._y = btn_2.submenu["tempo" + (contador2 - 1)]._y + btn_2.submenu["tempo" + (contador2 - 1)]._height;
        }
        else
        {
            mc._y = 0;
        } // end else if
        btn_2.submenu.bkg._height = mc._y + mc._height;
    } // end if
} // end of for...in
for (n in objMenu3.boton)
{
    if (n != "__atributos")
    {
        ++contador3;
        mc = hazBoton(objMenu3.boton[n].titulo.__value, objMenu3.boton[n].link.__value, contador3, btn_3);
        mc._x = 0;
        if (contador != 1)
        {
            mc._y = btn_3.submenu["tempo" + (contador3 - 1)]._y + btn_3.submenu["tempo" + (contador3 - 1)]._height;
        }
        else
        {
            mc._y = 0;
        } // end else if
        btn_3.submenu.bkg._height = mc._y + mc._height;
    } // end if
} // end of for...in
for (n in objMenu4.boton)
{
    if (n != "__atributos")
    {
        ++contador4;
        mc = hazBoton(objMenu4.boton[n].titulo.__value, objMenu4.boton[n].link.__value, contador4, btn_4);
        mc._x = 0;
        if (contador != 1)
        {
            mc._y = btn_4.submenu["tempo" + (contador4 - 1)]._y + btn_4.submenu["tempo" + (contador4 - 1)]._height;
        }
        else
        {
            mc._y = 0;
        } // end else if
        btn_4.submenu.bkg._height = mc._y + mc._height;
    } // end if
} // end of for...in
