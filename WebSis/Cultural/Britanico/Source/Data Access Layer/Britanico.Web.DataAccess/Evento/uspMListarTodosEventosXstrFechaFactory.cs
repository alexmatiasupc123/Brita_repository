...
namespace Britanico.Web.DataAccess
{
    ...
    /// </summary>
    internal class uspMListarTodosEventosXstrFechaFactory : IDomainObjectFactory<BEEvento>
    {
        public BEEvento Construct(IDataReader reader)
        {
            ...
            int even_aimgIndex = reader.GetOrdinal("even_aimg");
            if (!reader.IsDBNull(even_aimgIndex))
            {
                bEEvento.even_aimg = reader.GetString(even_aimgIndex);

            }

            int even_lfecIndex = reader.GetOrdinal("even_lfec");
            if (!reader.IsDBNull(even_lfecIndex))
            {
                bEEvento.even_lfec = reader.GetString(even_lfecIndex);
