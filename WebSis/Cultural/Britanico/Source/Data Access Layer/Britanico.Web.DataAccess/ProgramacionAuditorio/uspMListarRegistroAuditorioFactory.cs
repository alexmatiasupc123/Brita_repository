...
namespace Britanico.Web.DataAccess
{
    ...
    /// </summary>
    internal class uspMListarRegistroAuditorioFactory : IDomainObjectFactory<BEProgramacionAuditorio>
    {
        public BEProgramacionAuditorio Construct(IDataReader reader)
        {
            ...
            int prog_aimgIndex = reader.GetOrdinal("prog_aimg");
            if (!reader.IsDBNull(prog_aimgIndex))
            {
                bEProgramacionAuditorio.prog_aimg = reader.GetString(prog_aimgIndex);

            }



            int prog_lfecIndex = reader.GetOrdinal("prog_lfec");
            if (!reader.IsDBNull(prog_lfecIndex))
            {
                bEProgramacionAuditorio.prog_lfec = reader.GetString(prog_lfecIndex);
