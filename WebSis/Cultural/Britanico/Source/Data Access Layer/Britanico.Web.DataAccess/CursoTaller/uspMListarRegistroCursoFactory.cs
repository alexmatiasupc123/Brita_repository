...
namespace Britanico.Web.DataAccess
{
    ...
    /// </summary>
    internal class uspMListarRegistroCursoFactory : IDomainObjectFactory<BECursoTaller>
    {
        public BECursoTaller Construct(IDataReader reader)
        {
            ...
            int curs_destIndex = reader.GetOrdinal("curs_dest");
            if (!reader.IsDBNull(curs_destIndex))
            {
                bECursoTaller.curs_dest = reader.GetBoolean(curs_destIndex);
            ...
            int curs_aimgIndex = reader.GetOrdinal("curs_aimg");
            if (!reader.IsDBNull(curs_aimgIndex))
            {
                bECursoTaller.curs_aimg = reader.GetString(curs_aimgIndex);

            }

            int curs_lfecIndex = reader.GetOrdinal("curs_lfec");
            if (!reader.IsDBNull(curs_lfecIndex))
            {
                bECursoTaller.curs_lfec = reader.GetString(curs_lfecIndex);
