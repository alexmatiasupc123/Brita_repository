...
public partial class ListaEjemplares : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (Request.QueryString["pm"] != null)
            {
                ...
                ddlEstadoEjemplar.SelectedValue = HelpEncrypt.QueryString(querystringDESENCRYP, "xTe");
    ...
    /*private string ParametrosDelFiltro()
    {
        ...
        sParameters = sParameters + "&xCi=" + txtCodEjemplar.Text.Trim();
        sParameters = sParameters + "&xAu=" + ddlAutor.SelectedValue.ToString();
        sParameters = sParameters + "&xTm=" + ddlTema.SelectedValue.ToString();
        sParameters = sParameters + "&xTe=" + ddlEstadoEjemplar.SelectedValue.ToString();
    ...
    private void Seguridad()
    {
        ...
        List<RolesOpciones> lstRolesOpciones = this.Master.HelpOpcion_Permiso_Lista("ListaEjemplares.aspx");//ELVP:11-10-2011
        ...
        if (lstRolesOpciones != null)  
        {
            ...
            {
                lblTituloPagina.Text = "Lista de Ejemplares";
            }
    ...
    private void BuscarFiltro()
    {
        ...
        ListarEjemplares(txtCodItem.Text, CodTipoItem, CodSac, txtTitulo.Text, CodAutor, txtISBN.Text, txtISSN.Text, CodTema, CodEstadoEjemplar, txtCodEjemplar.Text);
    }
    private void ListarEjemplares(string pCodItem, string pCodArguTipoItem, string pCodSac, string pTitulo, string pCodArguAutor, string pISBN, string pISSN, string pCodArguTema, string pCodArguEstadoEjemplar, string pCodEjemplar)
    {
        ItemLogic oItemLogic = new ItemLogic();
        List<ItemEjemplar1> lista = oItemLogic.ListarEjemplar(pCodItem, pCodArguTipoItem, pCodSac, pTitulo, pCodArguAutor, pISBN, pISSN, pCodArguTema, pCodArguEstadoEjemplar, pCodEjemplar);
        /*if (string.IsNullOrEmpty(pCodSac.Trim()) == false)
            lista = lista.Where(x => x.TotalEjemplares > 0).ToList<ItemEjemplar1>();*/
        gvItems.DataSource = lista; 
        gvItems.DataBind();
        HelpGridView.Caption(ref gvItems,"Lista de Ejemplares",lista.Count.ToString());
