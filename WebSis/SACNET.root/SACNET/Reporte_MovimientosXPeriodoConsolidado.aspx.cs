...
public partial class Reporte_MovimientosXPeriodoConsolidado : System.Web.UI.Page
{
    ...
    protected void ibtnAceptar_Click(object sender, ImageClickEventArgs e)
    {
        ...
        string TipoReporte = ddlTipoReporte.SelectedItem.Value;

        lista = oReportesLogic.MovimientosXPeriodoConsolidado(CodSac, CodTipoMaterial, FechaInicio, FechaFin, CodTipoMovimiento);
        ReportDataSource rds = new ReportDataSource("Britanico_SacNet_BusinessEntities_Reportes", lista);
        rvRankingTitulos.Reset();
        rvRankingTitulos.LocalReport.DataSources.Clear();
        rvRankingTitulos.LocalReport.DataSources.Add(rds);
        if (TipoReporte == "G")
        {
            rvRankingTitulos.LocalReport.ReportPath = @"Reportes\MovimientosXPeriodoConsolidado_Grafico.rdlc";
        }
        else
        {
            rvRankingTitulos.LocalReport.ReportPath = @"Reportes\MovimientosXPeriodoConsolidado.rdlc";
        }
    ...
    {
        RolesOpciones oRolesOpciones = this.Master.HelpOpcion_Permiso("Reporte_MovimientosXPeriodoConsolidado.aspx");
