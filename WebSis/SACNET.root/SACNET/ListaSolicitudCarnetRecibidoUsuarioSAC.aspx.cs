...
public partial class ListaSolicitudCarnetRecibidoUsuarioSAC : System.Web.UI.Page
{
    ...
    void MessageBox1_OnConfirmClick(object sender, EventArgs e)
    {
        ...
        {
            ConfirmarCarneEntregadoUsuario();
        }
    }
    ...
    protected void gvSolicitudesImpresionCarnet_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        switch (e.CommandName)
        {
            ...
            case "Editar":
                ...
                break;
            case "Entrega":
                querystringENCRYP = HelpEncrypt.Encriptar("id=" + codsCodSolicitud + "&nv=04" + "&ver=5" + ParametrosDelFiltro());
                Response.Redirect("MantSolicitudImpresionCarnet.aspx?pm=" + querystringENCRYP);
                break;
            case "Ver":
                querystringENCRYP = HelpEncrypt.Encriptar("id=" + codsCodSolicitud + "&nv=04" + "&ver=1" + ParametrosDelFiltro());
                Response.Redirect("MantSolicitudImpresionCarnet.aspx?pm=" + querystringENCRYP);
                break;
            default:
                break;
    ...
    protected void gvSolicitudesImpresionCarnet_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (IOK.Row[7].ToString() == HelpConfiguration.AppSettings(HelpConfiguration.AppSett.CodArguEstadoCarneP04))
                ((ImageButton)e.Row.Cells[13].FindControl("btnEntrega")).Visible = true;
            else
                ((ImageButton)e.Row.Cells[13].FindControl("btnEntrega")).Visible = false;
        }
    }

    protected void chkRecibidoUsuarioTODOS_CheckedChanged(object sender, EventArgs e)
    {
        foreach (GridViewRow item in gvSolicitudesImpresionCarnet.Rows)
        {
            ...
            {
                TieneFoto = ((CheckBox)item.Cells[12].FindControl("chkRecibidoUsuarioSAC"));
                TieneFoto.Checked = TieneRecibidoTodos.Checked;
            }
        }
    }

    protected void ButtonConfirmaEntregaUsuario_Click(object sender, EventArgs e)
    {
        string mensaje = Validar();
        if (mensaje == "")
        {
            MessageBox1.ShowConfirm("¿ Confirma actualizar las solicitudes a estado carné recibido por usuario del SAC ?", HelpEvento.Guardar.ToString());
        }
        else
        {
    ...
    private string Validar()
    {
        ...
        SolicitudesAEntregarUsu(listaSolicitudCarnet);
    ...
    private void CargarPagina()
    {
        ...
        prm_sCodArguEstado1.SelectedValue = HelpConfiguration.AppSettings(HelpConfiguration.AppSett.CodArguEstadoCarneP04);
    ...
    {
        //RolesOpciones oRolesOpciones = this.Master.HelpOpcion_Permiso("ListaSolicitudCarnetRecibidoUsuarioSAC.aspx");
        List<RolesOpciones> lstRolesOpciones = this.Master.HelpOpcion_Permiso_Lista("ListaSolicitudCarnetRecibidoUsuarioSAC.aspx");//ELVP:11-10-2011
        ...
        if (lstRolesOpciones != null)
        {
            ...
            if (dtNuevo.Rows.Count > 0)
            {
                ...
                ButtonConfirmaEntregaUsuario.Enabled = Convert.ToBoolean(dtNuevo.Rows[0]["Flag_Nuevo"]);
            }
            ...
            //ButtonConfirmaEntregaUsuario.Enabled = oRolesOpciones.Flag_Nuevo;
            //BotonesEdicionLista1.LoadComplete();
    ...
    private void ConfirmarCarneEntregadoUsuario()
    {
        List<SolicitudCarnet> listaSolicitudCarnet = new List<SolicitudCarnet>();
        SolicitudesAEntregarUsu(listaSolicitudCarnet);
        if (listaSolicitudCarnet.Count > 0)
        {
            ...
            {
                oReturnValor = oSolicitudCarnetLogic.ActualizarSolicitudCarnet(xSolicitudCarnet, 4); //  4
            }
    ...
    private void SolicitudesAEntregarUsu(List<SolicitudCarnet> listaSolicitudCarnet)
    {
        foreach (GridViewRow item in gvSolicitudesImpresionCarnet.Rows)
        {
            if (item.RowType == DataControlRowType.DataRow)
            {
                CheckBox TieneFoto = ((CheckBox)item.Cells[11].FindControl("chkRecibidoUsuarioSAC"));
                if (TieneFoto.Checked)
