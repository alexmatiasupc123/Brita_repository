...
using Oxinet.Tools;
public partial class ListaTransferenciaEjemplares_Ingreso : System.Web.UI.Page
{
    ...
    protected void gvSolicitudTrasnferencia_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        switch (e.CommandName)
        {
            case "Editar":
                ...
                Response.Redirect("ListaEjemplares_VerificarIngreso.aspx?pm=" + querystringENCRYP, false);
                break;
    ...
    {
        //RolesOpciones oRolesOpciones = this.Master.HelpOpcion_Permiso("ListaTransferenciaEjemplares_Ingreso.aspx");
        List<RolesOpciones> lstRolesOpciones = this.Master.HelpOpcion_Permiso_Lista("ListaTransferenciaEjemplares_Ingreso.aspx");//ELVP:11-10-2011
    ...
    private void CargarConfiguraciones()
    {
        ...
        if (Master.HelpUsuariosSAC().CodLocalSAC != null)
        {
            ...
            {
                string CodSac = Master.HelpUsuariosSAC().CodLocalSAC.Trim();
                if (CodSac != null)
                {
                    if (string.IsNullOrEmpty(CodSac) == false)
                    {
                        ddlSacDestino.SelectedValue = CodSac;
                        ddlSacDestino.Enabled = false;
                        ddlEstadoTransferencia.SelectedValue = HelpConfiguration.AppSettings(HelpConfiguration.AppSett.CodArguEstadoTransferencia_Retiro);
