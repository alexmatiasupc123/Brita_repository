...
public partial class ReporteEjemplares_Usuario : System.Web.UI.Page
{
    ...
    protected void ibtnAceptar_Click(object sender, ImageClickEventArgs e)
    {
        ...
        lista = oReportesLogic.ListarEjemplar_Alumno(txtCodItem.Text, CodTipoItem, CodSac, txtTitulo.Text, CodAutor, string.Empty, string.Empty, CodTema, CodEstadoEjemplar, txtCodEjemplar.Text);
        ReportDataSource rds = new ReportDataSource("Britanico_SacNet_BusinessEntities_ItemEjemplar_Alumno", lista);

        rvEjemplares.LocalReport.DataSources.Clear();
        rvEjemplares.LocalReport.DataSources.Add(rds);
        rvEjemplares.LocalReport.ReportPath = @"Reportes\ListaEjemplares_Usuario.rdlc";
    ...
    {
        RolesOpciones oRolesOpciones = this.Master.HelpOpcion_Permiso("ReporteEjemplares_Usuario.aspx");
        if (oRolesOpciones != null)
        {
            lblTituloPagina.Text = oRolesOpciones.CodigoOpcionNombre;
            lblTituloPagina.Text = "Reporte de Ejemplares por Estado";
