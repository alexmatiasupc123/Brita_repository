...
public partial class Reporte_UsuariosXSacXPeriodo : System.Web.UI.Page
{
    ...
    protected void ibtnAceptar_Click(object sender, ImageClickEventArgs e)
    {
        ...
        lista = oReportesLogic.UsuariosXSacXPeriodo(CodSac, CodTipoUsuario, FechaInicio, FechaFin);
        ReportDataSource rds = new ReportDataSource("Britanico_SacNet_BusinessEntities_Reportes", lista);

        rvUsuariosXSacXPeriodo.LocalReport.DataSources.Clear();
        rvUsuariosXSacXPeriodo.LocalReport.DataSources.Add(rds);
        rvUsuariosXSacXPeriodo.LocalReport.ReportPath = @"Reportes\UsuariosXSacXPeriodo.rdlc";
        ...
        rvUsuariosXSacXPeriodo.LocalReport.SetParameters(Parametros);

        rvUsuariosXSacXPeriodo.LocalReport.Refresh();
    }
    ...
    {
        RolesOpciones oRolesOpciones = this.Master.HelpOpcion_Permiso("Reporte_UsuariosXSacXPeriodo.aspx");
